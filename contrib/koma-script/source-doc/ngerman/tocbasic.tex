...
\Macro{addtocontents}\important{\Macro{addtocontents},
  \DescRef{\LabelBase.cmd.addxcontentsline}} oder \DescRef{\LabelBase.cmd.addxcontentsline} vorgenommen. Darüber
...
wie in der Erklärung von \DescRef{\LabelBase.cmd.addtotoclist} beschrieben verfahren. Wird das
...
Beispiel erforderlich, weil das Argument von \DescRef{\LabelBase.cmd.tocbasic@extend@babel}
zwingend bereits expandiert sein muss. Siehe dazu auch die Erklärung zu
\DescRef{\LabelBase.cmd.tocbasic@extend@babel} in \autoref{sec:tocbasic.internals},
...
\DescRef{\LabelBase.cmd.addtotoclist} zu entnehmen. Wird ein leerer \PName{Besitzer} angegeben,
...
\DescPageRef{\LabelBase.cmd.setuptoc} zu finden.%
\EndIndexGroup
...
\DescRef{\LabelBase.cmd.tocbasic@starttoc} an Stelle von \Macro{@starttoc} verwendet. Näheres
zu \DescRef{\LabelBase.cmd.tocbasic@starttoc} und \Macro{@starttoc} ist
...
\DescPageRef{maincls-experts.cmd.addparttocentry}). Ist die entsprechend
...
\DescRef{\LabelBase.cmd.addxcontentsline} nur damit Anwendung finden. Näheres zu diesen
...
\end{lstcode}
  ...
  das Inhaltsverzeichnis ist bei der Anweisung \DescRef{\LabelBase.cmd.setuptoc},
  \DescPageRef{\LabelBase.cmd.setuptoc} zu finden.
...
mit Hilfe von \DescRef{\LabelBase.cmd.listoftoc*}, \DescRef{\LabelBase.cmd.listoftoc} oder \DescRef{\LabelBase.cmd.listofeachtoc}
...
verschiedene Eigenschaften, die bei der Anweisung \DescRef{\LabelBase.cmd.setuptoc} erläutert
...
\DescRef{\LabelBase.cmd.listoftoc} oder \DescRef{\LabelBase.cmd.listofeachtoc} wird als dieses Argument dann der
...
\item[\PValue{leveldown}] bedeutet, dass das Verzeichnis nicht mit der
  obersten Gliederungsebene unterhalb von \DescRef{maincls.cmd.part} -- wenn vorhanden
  \DescRef{maincls.cmd.chapter}, sonst \DescRef{maincls.cmd.section} -- erstellt wird, sondern mit einer
  ...
  hingegen eine eigene Überschriftenanweisung mit \DescRef{\LabelBase.cmd.deftocheading}
  ...
  \OptionValueRef{maincls}{listof}{leveldown}\important{\OptionValueRef{maincls}{listof}{leveldown}}%
...
\item[\PValue{numbered}] bedeutet, dass das Verzeichnis nummeriert und damit
  ...
  \IndexOption{listof~=\PValue{numbered}} für alle Dateierweiterungen des
...
\item[\PValue{numberline}] \ChangedAt{v3.12}{\Package{tocbasic}}%
  ...
  \DescRef{\LabelBase.cmd.addxcontentslinetoeachtocfile} in das Verzeichnis
  geschrieben werden und bei denen das optionale Argument für die Nummer fehlt
...
\item[\PValue{totoc}] bedeutet, dass der Titel des Verzeichnisses in das
  ...
  \IndexOption{listof~=\PValue{totoc}} für alle Dateierweiterungen des
  Besitzers \Package{float}.
\end{description}
...
\end{lstcode}
  Wollen Sie außerdem, dass bei Klassen, die \DescRef{maincls.cmd.chapter} als oberste
...
\item[\PValue{largetocline}] entspricht dem Stil, der von den Standardklassen
  ...
  ausgegeben. Sollte \DescRef{\LabelBase.cmd.numberline} verwendet werden, so
  ist die Nummernbreite fest auf 3\Unit{em}
...
\item[\PValue{tocline}] ist ein flexibler Stil, der in der Voreinstellung für
  ...
  Standardklassen mit \DescRef{\LabelBase.cmd.DeclareTOCEntryStyle} in \PValue{tocline} zu
...
\item[\PValue{undottedtocline}] entspricht dem Stil, der von den
  ...
  eingezogen in \Macro{bfseries}
  ausgegeben. \DescRef{\LabelBase.cmd.numberline} wird nicht umdefiniert. Die
  Breite der Nummer wird von \PValue{numwidth} bestimmt. Bei mehrzeiligen
  Einträgen wird der Einzug ab der zweiten Zeile um \PValue{numwidth}
  erhöht. \autoref{fig:tocbasic.undottedtocline} illustriert die Eigenschaften
  des Stils.
  \begin{figure}
    \centering
    \resizebox{.8\linewidth}{!}{%
...
als Optionen für \DescRef{\LabelBase.cmd.DeclareNewTOC} (siehe
...
an Stelle von \PValue{level} zu
verwenden. Bei\ChangedAt[2015/12]{v3.20}{\Package{tocbasic}} Verwendung als
Optionen für \DescRef{maincls-experts.cmd.DeclareSectionCommand} (siehe
...
\begin{desclist}
  ...
  \entry{\OptionVName{breakafternumber}{Schalter}}{%
    ...
    Ist\textnote{Achtung!} für ein Verzeichnis mit
    \DescRef{\LabelBase.cmd.setuptoc} die Eigenschaft \Option{numberline}
    gesetzt (siehe \autoref{sec:tocbasic.toc},
    \DescPageRef{\LabelBase.cmd.setuptoc}), wie dies bei den \KOMAScript-Klassen
    und Verwendung deren Option \OptionValueRef{maincls}{toc}{numberline}%
    \IndexOption{toc~=\PValue{numberline}} der Fall ist, führt dies auch dazu,
    dass bei nicht nummerierten Einträgen dennoch die Zeile mit der dann
  ...
  \entry{\OptionVName{entrynumberformat}{Befehl}}{%
    ...
    und Verwendung deren Option \OptionValueRef{maincls}{toc}{numberline}%
    \IndexOption{toc~=\PValue{numberline}} der Fall ist, führt dies auch dazu,
    dass bei nicht nummerierten Einträgen \PName{Befehl} dennoch ausgeführt
    wird.%
  }%
  ...
  \entry{\OptionVName{onstarthigherlevel}{Code}}{%
    ...
    Initialisiert wird die Eigenschaft mit \DescRef{\LabelBase.cmd.LastTOCLevelWasLower} (siehe
  ...
  \entry{\OptionVName{onstartlowerlevel}{Code}}{%
    ...
    Initialisiert wird die Eigenschaft mit \DescRef{\LabelBase.cmd.LastTOCLevelWasHigher} (siehe
  ...
  \entry{\OptionVName{onstartsamelevel}{Code}}{%
    ...
    Initialisiert wird die Eigenschaft mit \DescRef{\LabelBase.cmd.LastTOCLevelWasSame} (siehe
    \DescPageRef{\LabelBase.cmd.LastTOCLevelWasSame}), was normalerweise
    dazu führt, dass ein Umbruch vor dem Eintrag begünstigt wird.%
  }%
...
\DescRef{\LabelBase.cmd.DeclareTOCStyleEntry} der \PName{Stil} zugewiesen, so
wird zunächst der optionale \PName{Initialisierungscode} ausgeführt und dann
...
\DescRef{\LabelBase.cmd.DeclareTOCStyleEntry} auch ohne Angabe einer
\PName{Optionenliste} fehlerfrei funktioniert. Darüber hinaus hat der
...
\DescRef{\LabelBase.cmd.TOCLineLeaderFill} zu definieren. Ein Aufruf wie
\begin{lstcode}
...
mit \DescRef{\LabelBase.cmd.CloneTOCEntryStyle} zu klonen und dann dessen
Initialisierungscode mit \DescRef{\LabelBase.cmd.TOCEntryStyleInitCode} oder
...
zugewiesen wird. Dieser \PName{Initialisierungscode} sollte keine globalen
Seiteneffekte aufweisen, da er auch für lokale Initialisierungen innerhalb
anderer Anweisungen wie \DescRef{\LabelBase.cmd.DeclareNewTOC}\IndexCmd{DeclareNewTOC}
verwendet wird. Der \PName{Initialisierungscode} dient einerseits dazu,
Eigenschaften für den jeweiligen \PName{Stil} zu definieren. Er setzt aber
auch die Standardeinstellungen für diese Eigenschaften.
...
\LaTeX-Kern. Es ist die Anweisung, die sich hinter \DescRef{\LabelBase.cmd.listoftoc*} (siehe
\autoref{sec:tocbasic.toc}, \DescPageRef{\LabelBase.cmd.listoftoc*})
...
\DescRef{\LabelBase.cmd.listoftoc} verwenden. Die Anweisung baut selbst auf \Macro{@starttoc}
...
Verzeichnisdatei, noch vor den mit \DescRef{\LabelBase.cmd.BeforeStartingTOC}
definierten Anweisungen ausgeführt. Es ist erlaubt, diesen Haken mit Hilfe von
...
\end{Declaration}
Diese Anweisungen werden direkt nach \DescRef{\LabelBase.cmd.tocbasic@@before@hook} bzw. vor
\DescRef{\LabelBase.cmd.tocbasic@@after@hook} für das jeweilige Verzeichnis mit der
...
die Ausgabe der Verzeichnisse die Anweisungen
\DescRef{\LabelBase.cmd.listoftoc}, \DescRef{\LabelBase.cmd.listoftoc*} und
\DescRef{\LabelBase.cmd.listofeachtoc} nicht verwendet, sollten die beiden
Anweisungen für die Haken trotzdem aufgerufen werden, soweit sie definiert
sind. Die Anweisungen können auch undefiniert sein. Für einen entsprechenden
Test siehe
\DescRef{scrbase.cmd.scr@ifundefinedorrelax}\IndexCmd{scr@ifundefinedorrelax}
in \autoref{sec:scrbase.if}, \DescPageRef{scrbase.cmd.scr@ifundefinedorrelax}.
%
\EndIndexGroup
...
definiert, so verwendet \DescRef{\LabelBase.cmd.tocbasic@listhead} diese. Anderenfalls
definiert \DescRef{\LabelBase.cmd.tocbasic@listhead} diese und ruft sie dann auf.
%
\EndIndexGroup
...
Argument vorangestellt. Anderenfalls wird wie gewohnt
\DescRef{\LabelBase.cmd.numberline} mit \PName{Gliederungsnummer} als Argument
verwendet.
...
Setzen oder Löschen dieser Eigenschaft bereits beim nächsten \LaTeX-Lauf aus.%
%
\EndIndexGroup
...
\autoref{tab:tocbasic.DeclareNewTOC-options} ab

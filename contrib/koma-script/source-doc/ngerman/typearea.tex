...
werden. Wird\textnote{automatische Neuberechnung} die Option per
\DescRef{\LabelBase.cmd.KOMAoptions} oder \DescRef{\LabelBase.cmd.KOMAoption}
nach dem Laden des Pakets geändert, so werden Satzspiegel und Rändern
automatisch neu berechnet.
...
\OptionVName{\DescRef{\LabelBase.option.DIV}}{Faktor}.  Selbstverständlich können Sie diese Option auch
...
\begin{Example}
  In dem bei der Option \OptionVName{\DescRef{\LabelBase.option.DIV}}{Faktor} aufgeführten
...
\begin{Example}
  ...
  \PValue{last} und der Anweisung \DescRef{\LabelBase.cmd.KOMAoptions} sehr einfach möglich:
\begin{lstcode}
...
\begin{table}
  \caption[{Symbolische Werte für Option \Option{DIV} und das
    \PName{DIV}-Argument von \DescRef{typearea.cmd.typearea}}]{Mögliche symbolische Werte
...
\begin{Explain}
  ...
  aufgerufen. Dabei wird beim Setzen der Option \DescRef{\LabelBase.option.DIV} für \PName{BCOR}
  intern der symbolische Wert \PValue{current} verwendet, der aus Gründen der
  Vollständigkeit auch in \autoref{tab:symbolicBCOR} zu finden ist. Beim
  Setzen der Option \DescRef{\LabelBase.option.BCOR} wird für \PName{DIV}
  hingegen der symbolische Wert \PValue{last} verwendet. Wollen Sie, dass
  Satzspiegel und Rändern stattdessen mit dem symbolischen Wert
  \PValue{current} für \PName{DIV} neu berechnet werden, so können Sie direkt
...
\PParameter{\OptionVName{\DescRef{\LabelBase.option.DIV}}{DIV},%
  \OptionVName{\DescRef{\LabelBase.option.BCOR}}{BCOR}} Ränder und Satzspiegel
zunächst bei der Änderung für \PName{DIV} und dann zusätzlich bei der Änderung
für \PName{BCOR} neu berechnet.
...
Klassen und Paketen für die Gestaltung von Kopf und Fuß verwendet wird, hängt
von den verwendeten Klassen und Paketen ab. Das \KOMAScript-Paket
\Package{scrlayer-scrpage} berücksichtigt
\Length{footheight} und arbeitet somit aktiv mit
\Package{typearea} zusammen. Die \KOMAScript-Klassen berücksichtigen
...
\important{\OptionVName{paper}\PName{Breite}\texttt{:}{Höhe}}
beziehungsweise
\PName{Höhe}\texttt{:}\PName{Breite}\ChangedAt{v3.22}{\Package{typearea}}%
\important{\OptionVName{paper}{Höhe}\texttt{:}\PName{Breite}} angegeben
...
und der größere Wert die \PName{Höhe}.
...
\cite{package:geometry}) oder verändern Sie die Satzspiegelparameter von
...
möglich sein. Achten Sie jedoch unbedingt darauf, dass die Proportionen des
Textbereichs mit denen der Seite unter Berücksichtigung der Bindekorrektur
...
\OptionValueRef{\LabelBase}{DIV}{calc} einen sehr großzügigen

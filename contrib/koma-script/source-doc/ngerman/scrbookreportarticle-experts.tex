...
\DescPageRef{maincls-experts.option.fontsize} für die
...
Anweisungen können mit \Macro{addtokomafontrelaxlist} einzeln hinzugefügt
werden.
...
\DescRef{maincls.cmd.usefontofkomafont} und ihren Geschwistern einschließen
...
genau dann aus, wenn das \PName{Element} über
...
\item[\PValue{chapter}:] \ChangedAt{v3.18}{\Class{scrbook}\and
  ...
  \DescRef{maincls.cmd.addchap}\IndexCmd{addchap} verwendet. Neue Überschriften in diesem
  ...
  Verfügung. Die\textnote{Achtung!} Anweisung \DescRef{maincls.cmd.addchap} wird ebenso wie
...
\item[\PValue{section}:] Stil\textnote{Gliederungsstil
  ...
  \DescRef{maincls.cmd.subparagraph}\IndexCmd{subparagraph} verwendet. Neue Überschriften in
  ...
  \DescRef{maincls.cmd.addtokomafont} (siehe \autoref{sec:maincls.textmarkup},
  \DescPageRef{maincls.cmd.setkomafont}) geändert werden kann, falls ein
...
\begin{table}
  ...
  \begin{tabularx}{\linewidth}{llX}
    ...
    \midrule \PValue{counterwithin} & \PName{Zählername} & Der zur
    Gliederungsebene gehörende Zähler soll vom als Wert angegebenen Zähler
    abhängig sein. Wird \PName{Zählername} über \Macro{stepcounter} oder
    \Macro{refstepsounter} erhöht, so wird der zur Gliederungsebene gehörende
    Zähler auf 0 zurückgesetzt. Darüber hinaus wird
    \Macro{the\PName{Zählername}} gefolgt von einem Punkt in der Ausgabe des
    ...
    Änderung vorhandener Gliederungsbefehle
    sinnvoll.\\
    \PValue{expandtopt} & \PName{Schalter} & Ist dieser Schalter aktiv, so
    werden alle in den \PName{Einstellungen} nachfolgend angegebenen Werte für
    Längen vollständig expandiert, ausgewertet und in \texttt{pt} umgerechnet
    gespeichert.  Ist der Schalter nicht aktiv, so werden alle nachfolgend
    angegebenen Werte für Längen nur testweise expandiert und ausgewertet aber
    ...
    (siehe Zähler \DescRef{maincls.counter.secnumdepth},
    ...
    \ChangedAt[2016/03]{v3.20}{\Class{scrbook}\and \Class{scrreprt}\and
      \Class{scrartcl}}%
    \PValue{tocstyle} & \PName{Name} & Legt den Stil der Verzeichniseinträge
    fest. Es können alle bisher definierten Verzeichniseintragsstile (siehe
    \autoref{sec:tocbasic.tocstyle}) verwendet werden. Ein leerer \PName{Name}
    verhindert die Definition des Befehls für
    Verzeichniseinträge.\\
    \ChangedAt[2016/03]{v3.20}{\Class{scrbook}\and \Class{scrreprt}\and
      \Class{scrartcl}}%
    \PValue{toc\PName{Option}} & \PName{Wert} & Weitere Optionen in
    Abhängigkeit vom via \Option{tocstyle} gewählten
...
\begin{Example}
  Aus unerfindlichen Gründen sollen die Überschriften von \DescRef{maincls.cmd.paragraph}
  nicht mehr als Spitzmarken, sondern als Überschriften ähnlich
  \DescRef{maincls.cmd.subsubsection} umdefiniert werden. Dabei soll über der Überschrift
...
\begin{table}
  ...
  \caption{Voreinstellungen für die Kapitelüberschriften von \Class{scrbook}
    und \Class{scrreprt} in Abhängigkeit von Option \DescRef{maincls.option.headings}}
...
Überschriften \DescRef{maincls.cmd.chapter}\IndexCmd{chapter}, \DescRef{maincls.cmd.part},
...
Die Anweisung \DescRef{maincls.cmd.chapter} setzt die interne Länge
...
die Abstände über, innerhalb und unter den Überschriften wesentlich einfacher
...
nachträgliche Umdefinierung von \Macro{secdef} und \Macro{@startsection} hat
...
Anweisung von \DescRef{maincls.cmd.appendix} ebenfalls ausgeführt. Intern wird

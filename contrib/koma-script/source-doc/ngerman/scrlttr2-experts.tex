...
\begin{desclist}
  ...
  \pentry{PPheadwidth}{%
    Breite des linken Port-Payé-Feldes bei \OptionValueRef{scrlttr2}{addrfield}{PP}
    (\autoref{sec:scrlttr2-experts.addressee},
...
normalerweise voreingestellten Wert von 0\Unit{mm} ändert \Class{scrlttr2}
selbständig in 9\Unit{mm}. Die Data-Matrix wird rechtsbündig zum
Port-Payé-Kopf gesetzt.%
\EndIndexGroup
%
\EndIndexGroup
...
\autoref{sec:scrlttr2.firstpage},
\DescPageRef{scrlttr2.option.enlargefirstpage}) kann dafür gesorgt
...
\Package{scrlayer-scrpage} auch für
...
den Kopf und Fuß der Seite bestimmt. Da dieser Seitenstil bei den
...
Durch diese interne Verwendung von \Package{scrlayer-scrpage} können die
...
\Package{scrletter} verwendet werden. Darüber hinaus wird durch das interne
Laden von \Package{scrlayer-scrpage} automatisch der Seitenstil
\PageStyle{scrheadings}\IndexPagestyle{scrheadings} aktiviert. Dies führt
...
definiert, um unabhängig von den Seitenstilen der Klasse zu sein. Dabei werden
die Variablen \DescRef{scrlttr2.variable.nexthead} und
\DescRef{scrlttr2.variable.nextfoot} für \PageStyle{letter} so verwendet, wie
dies in \autoref{sec:scrlttr2.pagestyle} für den Seitenstil
\PageStyle{headings} dokumentiert ist. Der zugehörige
...
in Kopf und Fuß des \PageStyle{plain}-Seitenstils
...
Paginierung der Briefe, sondern alle Paginierungen betreffen würde, kann
...
\Macro{begin}\PParameter{letter} nur für die Dauer des Briefs auf die
Definition von \Macro{letterpagemark}. Will man daher \Macro{pagemark} sowohl
für das normale Dokument als auch für die Briefe umdefinieren, so sollte man
nach der normalen Umdefinierung von \Macro{pagemark} noch
\begin{lstcode}
  \let\letterpagemark\pagemark
\end{lstcode}
ergänzen. Diese Anweisung stört im übrigen auch nicht, wenn man später auf die
...
\Class{scrlttr2} jedoch nicht der Fall. Das Paket \Package{french} zerstört
...
\Class{scrlttr2} werden die Begriffe erst nach der Präambel, also bei

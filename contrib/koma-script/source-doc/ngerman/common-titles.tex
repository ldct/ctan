...
auf einer eigenen Seite. Neben der Haupttitelseite kann es dabei weitere
...
{\KOMAScript} wurde die Titelei gegenüber den Standardklassen stark erweitert.
...
\DescRef{\LabelBase.cmd.maketitle}\IndexCmd{maketitle} ausgegebene Titelseite, also entweder
der Schmutztitel oder der Haupttitel, als Umschlagseite\Index{Umschlag}
ausgegeben wird. Jede andere Einstellung für die Option \Option{titlepage}
hebt diese Einstellung wieder auf. Die Ränder\important{%
...
\DescRef{maincls.cmd.thispagestyle}\PParameter{empty} geändert,
so dass weder Seitenzahl noch Kolumnentitel ausgegeben werden. Am Ende der
Umgebung wird die Seite automatisch beendet. Sollten Sie nicht das
...
\IfThisCommonLabelBase{scrextend}{}{drei }Angaben Titel, Autor und Datum
existiert, können bei {\KOMAScript} mit \Macro{maketitle} bis zu sechs
Titelseiten gesetzt werden.  Im Gegensatz zu den Standardklassen kennt
\Macro{maketitle} bei {\KOMAScript} außerdem noch ein optionales nummerisches
Argument.  Findet es Verwendung, so wird die Nummer als Seitenzahl der ersten
Titelseite benutzt. Diese Seitenzahl wird jedoch nicht ausgegeben, sondern
beeinflusst lediglich die Zählung. Sie sollten hier unbedingt eine ungerade
Zahl wählen, da sonst die gesamte Zählung durcheinander gerät. Meiner
Auf"|fassung nach gibt es nur zwei sinnvolle Anwendungen für das optionale
Argument. Zum einen könnte man dem Schmutztitel\Index[indexmain]{Schmutztitel}
die logische Seitenzahl \(-\)1 geben, um so die Seitenzählung erst ab der
Haupttitelseite mit 1 zu beginnen. Zum anderen könnte man mit einer höheren
Seitenzahl beginnen, beispielsweise 3, 5 oder 7, um so weitere Titelseiten zu
berücksichtigen, die erst vom Verlag hinzugefügt werden. Wird ein Titelkopf
verwendet, wird das optionale Argument ignoriert. Dafür kann der Seitenstil
einer solchen Titelei durch Umdefinierung des Makros
...
gestaltet werden. Für die Ausgabe wird die Schrift des gleichnamigen Elements
...
\PName{Titel} in der Schrift des gleichnamigen Elements ausgegeben.%
\EndIndexGroup
...
ausgegeben.  Dabei\IfThisCommonLabelBase{maincls}{%
  \ChangedAt{v2.8p}{\Class{scrbook}\and \Class{scrreprt}\and
    \Class{scrartcl}}}{} findet \IfThisCommonLabelBase{scrextend}{}{abgesehen
  von der Größe }% Umbruchkorrektur
Schriftgrößenumschaltungen für das Element
\FontElement{title}\IndexFontElement[indexmain]{title}%
\important{\FontElement{title}} keine Wirkung (siehe
...
\PName{Untertitel}\ChangedAt{v2.97c}{\Class{scrbook}\and \Class{scrreprt}\and
  \Class{scrartcl}} wird  knapp unter dem Titel in der Schrift des
gleichnamigen Elements ausgegeben (siehe
\autoref{tab:\ThisCommonFirstLabelBase.mainTitle},
\autopageref{tab:\ThisCommonFirstLabelBase.mainTitle}).%
\EndIndexGroup
...
trennen. Die Ausgabe erfolgt in der Schrift des gleichnamigen Elements (siehe
\autoref{tab:\ThisCommonFirstLabelBase.mainTitle},
\autopageref{tab:\ThisCommonFirstLabelBase.mainTitle}).%
\EndIndexGroup
...
erfolgt in der Schrift des gleichnamigen Elements (siehe
\autoref{tab:\ThisCommonFirstLabelBase.mainTitle},
\autopageref{tab:\ThisCommonFirstLabelBase.mainTitle}).%
\EndIndexGroup
...
vorhandenen Fußnoten ausgegeben wird. Die Ausgabe erfolgt in der Schrift des
...
} die Ausgabe der Titelelemente kann die Schrift über gleichnamige Elemente
    ...
    \begin{captionbeside}
      ...
      \begin{tabular}[t]{ll}
        ...
        \FontElement{subtitle} &
          \DescRef{\ThisCommonLabelBase.cmd.usekomafont}\PParameter{title}\Macro{large} \\
        \FontElement{title} & 
          \DescRef{\ThisCommonLabelBase.cmd.usekomafont}\PParameter{disposition} \\
...
horizontal zentriert. \IfThisCommonLabelBase{scrextend}{Die Formatierungen der
  einzelnen Elemente}{Diese Angaben} sind noch einmal kurz zusammengefasst in
...
\begin{Explain}
  ...
  \IndexOption{titlepage~=\PValue{firstiscover}} auf
...
\DescRef{\ThisCommonLabelBase.cmd.maketitle} ausgegeben und muss daher vor
dieser Anweisung definiert sein.
